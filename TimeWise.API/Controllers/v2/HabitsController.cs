...
namespace TimeWise.API.Controllers.v2
{
    /// <summary>
    /// Habits - Versão 2.0
    /// </summary>
    ...
    public class HabitsController : ControllerBase
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(dto.Titulo)) return BadRequest("Titulo é obrigatório");
            ...
            _logger.LogInformation("Habit created v2.0 {Id}", created.Id);

            var read = ToReadDto(created);
            return CreatedAtAction(nameof(GetById), new { id = created.Id, version = "2.0" }, read);
        }

        /// <summary>
        /// Obtém um hábito específico pelo ID (v2.0)
        /// </summary>
        ...
        /// <summary>
        /// Lista hábitos com paginação e filtro opcional por usuário (v2.0)
        /// </summary>
        ...
        /// <summary>
        /// Atualiza um hábito existente (v2.0)
        /// </summary>
        ...
        private HabitReadDto ToReadDto(Habit h)
        {
            ...
            {
                new { rel = "self", href = Url.Action(nameof(GetById), new { id = h.Id, version = "2.0" }), method = "GET" },
                new { rel = "update", href = Url.Action(nameof(Update), new { id = h.Id, version = "2.0" }), method = "PUT" },
                new { rel = "delete", href = Url.Action(nameof(Delete), new { id = h.Id, version = "2.0" }), method = "DELETE" }
            };
